...
- Phase 10 does two runs: a fake-adapter demo through `leg up` on a scratch
...
- To resume anything: `node bin/leg.mjs up` (board), `node bin/leg.mjs card ls`,
  `node bin/leg.mjs card show <id>`, `node bin/leg.mjs card run <id>` (a run
