...
export async function run({ id, card, station, worktree, actor, ops }) {
  const { resolveTestCommand, runTestCommand, ledgerAppend, step, transition, handoffOn, apply } = ops
  ...
  const t = runTestCommand(cmd, worktree)
  ...
  if (!t.green) {
    ...
    handoffOn(card, { name: station.name, chain: [] }, { outcome: 'test_red', reason: t.tail, exit_code: t.status, adapter: 'test' }, worktree, [`test red: ${t.command}`, t.tail.slice(0, 1500)])
    return { card: apply(id, card, bounced, actor) }
  }
  return { card: step(id, 'test_result', { green: true }, actor) }
}
