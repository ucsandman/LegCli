...
Email **legcli@practicalsystems.io** with `SECURITY` in the subject. The
...
Leg is local-first by design:

- The board server binds `127.0.0.1` by default. It does not listen on any
  other address unless you explicitly set `LEG_BIND`, and it refuses to
  start on a non-loopback address without `LEG_TOKEN` also set (exit code
  `3`; see [docs/configuration.md](docs/configuration.md#network-exposure)).
- The token seam (`LEG_TOKEN`, `Authorization: Bearer <token>`, or
...
- Every adapter spawns its CLI as `argv`, never a shell, and strips
  ...
  or to make Leg fall back to a shell spawn anywhere, is in scope.
...
machine Leg runs on, and vulnerabilities in the coding-agent CLIs
