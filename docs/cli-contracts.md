...
<!-- limits-table:start -->
Generated by `node scripts/limits-table.mjs` from 25 fixtures (5 observed-live, 20 docs-only). Classification `limit` hands the card to the next agent as a usage limit; `auth` is a failed launch (never a limit); `launch` is a failed launch that the next agent may still try; `budget` is a turn or spend cap set by Baton itself; `info` must never classify as a limit.
...
| claude-max-turns | claude | budget | any | **docs-only** | (no message text documented; the run ends with an error once --max-turns is reached; phase | https://code.claude.com/docs/en/cli-reference (--max-turns: "Exits with an error when the limit is reached") |
