...
Filled in phase 4 from recorded fixtures and each CLI's own documentation. Every
signal is tagged observed-live or docs-only. Until then the runner classifies
nothing: `run.json.outcome` is `null`.
