...
- [VOCABULARY.md](VOCABULARY.md): the exact identifiers Baton uses for session statuses and session events, and for card statuses, outcomes, event types, actor types, human actions and bounce reasons.
- [cli-contracts.md](cli-contracts.md): what Baton knows about each coding-agent CLI, the interactive taps and their sources, and the limit-signal table, every fact tagged observed-live or docs-only.
...
| area | modules |
|------|---------|
| interactive sessions | `src/attach.mjs` (the `baton <agent>` runner), `src/sessions.mjs` (the session store), `src/usage.mjs` (usage windows and the chooser), `src/accounts.mjs` (extra logins), `src/bundle.mjs` (the per-session bundle), `src/hook.mjs` (what Claude Code's hooks run) |
