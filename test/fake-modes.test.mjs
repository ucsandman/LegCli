...
for (const [mode, adapter, extra, outcome, handoff, exit, eventType] of EXPECT) {
  ...
  test(`fake mode ${label} → ${outcome}`, (t) => {
    ...
    const events = readFileSync(join(root, 'cards', id, 'events-baton.jsonl'), 'utf8').trim().split('\n').map(JSON.parse)
    ...
    if (mode === 'success') assert.ok(existsSync(join(work, 'hello-fake.txt')) && existsSync(join(work, '.baton', 'DONE')))
    if (mode === 'incomplete') assert.ok(existsSync(join(work, 'hello-fake.txt')) && !existsSync(join(work, '.baton', 'DONE')))
