...
test('supervise: fast finish -> exited 0, session id stored on run and card, no timer events', () => {
  ...
  assert.equal(r.done_marker, true)
  ...
  assert.ok(evs.some((e) => e.type === 'leg_exited' && /completed \(exit 0/.test(e.summary)))
...
test('supervise: runaway agent -> still-running status event then killed, exit 12', () => {
  ...
  assert.equal(rj.outcome, 'stalled')
  assert.equal(rj.handoff, true)
  const evs = batonEvents(root, id)
  assert.ok(evs.some((e) => e.type === 'status' && /still running/.test(e.summary)))
  assert.ok(evs.some((e) => e.type === 'killed' && /stalled/.test(e.summary)))
})
...
test('supervise: child env is sanitized of every forbidden key and the print ceiling is 0', () => {
  ...
  const id = makeCard(root, env)
  prepRun(root, id)
  const work = join(root, 'work') // apart from BATON_HOME so state writes never look like work
  mkdirSync(work)
  runFail(['supervise', '--card', id, '--adapter', 'fake', '--run', '1', '--cwd', work], env) // envcheck does no work: exits 13 (no_progress)
  assert.equal(runJson(root, id).session_id, 'clean-env')
  assert.equal(runJson(root, id).outcome, 'no_progress')
})
